 ...
 * feed entries on the page.
 * @param {function} id - Feeds are referenced by their index position within
 * the allFeeds array.
 * @param {function} cb - This function all supports a callback as the
 * second parameter which will be called after everything has run successfully.
 */
function loadFeed(id, cb) {
  let feedUrl = allFeeds[id].url;

  let feedName = allFeeds[id].name;
  ...
  $.ajax({
    ...
    success: function(result, status) {
      ...
      let entryTemplate = Handlebars.compile($('.tpl-entry').html());
